    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#ABB8C4] mt-1">{doctors.length} verified doctors available</p>
        </div>
      </div>

      {doctors.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <Stethoscope size={48} className="text-[#363A3D]" />
          <p className="text-[#76828D]">No approved doctors yet. Check back soon.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {doctors.map(doc => (
            <div key={doc.id} className="bg-[#161A1F] border border-[#1E2124] rounded-2xl overflow-hidden hover:border-[#24AE7C]/30 transition-all group">
              {/* Header */}
              <div className="relative h-40 bg-gradient-to-br from-[#24AE7C]/10 to-[#1E2124]">
                {doc.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Stethoscope size={40} className="text-[#363A3D]" />
                  </div>
                )}
              ...
              <div className="p-5 space-y-3">
                ...
                {doc.doctorProfile?.bio && (
                  <p className="text-xs text-[#76828D] line-clamp-2">{doc.doctorProfile.bio}</p>
                )}
                ...
                {doc.doctorProfile?.availableDays && doc.doctorProfile.availableDays.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {doc.doctorProfile.availableDays.map(day => (
                      <span key={day} className="text-xs bg-[#0D0F10] border border-[#1E2124] text-[#ABB8C4] rounded-full px-2 py-0.5">
                        {day.slice(0, 3)}
                      </span>
                    ))}
                  </div>
