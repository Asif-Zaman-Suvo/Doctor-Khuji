...
export default function CancelButton({ appointmentId }: { appointmentId: string }) {
  ...
  if (confirm) {
    return (
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setConfirm(false)} className="text-xs text-app-subtle hover:text-app-text cursor-pointer">
          No
        </button>
      </div>
    );
  ...
  return (
    <button
      ...
      className="flex items-center gap-1 text-xs text-app-subtle hover:text-red-400 border border-app-border hover:border-red-500/20 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
    >
      <X size={12} /> Cancel
    </button>
  );
